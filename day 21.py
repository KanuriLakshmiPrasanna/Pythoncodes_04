...
4
Explanation:
Test Case 1: For each bill payment, one receives 10 CRED coins. Chef pays 10 bills using CRED. Thus, he receives 100 CRED coins.
 Chef can get 1 bag from the CodeChef store using these coins.
...
Test Case 3: For each bill payment, one receives 
70 CRED coins. Chef pays 7 bills using CRED. Thus, he receives 490 CRED coins. Chef can get at most 4 bags from the CodeChef store using these coins.'''


for _ in range(int(input())):
    X,Y=map(int,input().split())
    Cred_coins=X*Y
    max_bags=int(Cred_coins/100)
    print(max_bags)