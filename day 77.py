...
Each test case contains six space-separated integers A 1, A2,A3,B1,B2.B3  — the values Alice gets in her  dice rolls, followed by the values which Bob gets in his 3 dice rolls.
Output Format
...
Tie
Explanation:
Test Case 1: Alice's score is 
8=(3+5) which is greater than Bob's score 7=(6+1).
...
for _ in range(int(input())):
    ...
    score_of_alice=(A1+A2+A3)-min(A1,A2,A3)
    score_of_bob=(B1+B2+B3)-min(B1,B2,B3)
    if score_of_alice<score_of_bob:
        print("Bob")
    elif score_of_alice>score_of_bob:
        print("Alice")
    else:
        print("Tie")
