...
for _ in range(T):
    N, K = map(int, input().split())      # N = number of days, K = daily protein requirement
    A = list(map(int, input().split()))   # List of proteins bought each day
    
    store = 0  # Initial protein storage
    enough = True

    for i in range(N):
        store += A[i]  # Add today's protein
        if store >= K:
            store -= K  # Use K grams for dinner
        else:
            print("NO", i + 1)  # Not enough protein on day i+1
            enough = False
            break

    if enough:
        print("YES")  # Enough protein for all days